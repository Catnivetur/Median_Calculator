...
int main(int argc, char* argv[])
{
    ...
    if (!config_parser::directory_works(csv_output_dir))
    {
        if (!config_parser::create_directory(csv_output_dir))
        {
            ...
            if(!config_parser::create_directory(csv_output_dir))
            {
                ...
                spdlog::info("Created output directory at {}", csv_output_dir);
            }
        }
        else {
            config_parser::directory_works(csv_output_dir);
    ...
    // {
    //     auto point1 = std::chrono::high_resolution_clock::now();
    //     auto data1 = csv_reader::read_all_csv_files(csv_files, {"receive_ts", "price"});
    //     auto point2 = std::chrono::high_resolution_clock::now();
    //     auto data2 = csv_reader::read_all_csv_files_parallel(csv_files, {"receive_ts", "price"});
    ...
    //     spdlog::info("Size: {}, {}", data1.size(), data2.size());
    //     return 0;
    // }
